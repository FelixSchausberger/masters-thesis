...
\begin{tikzpicture}[scale = 1.25]%
    ...
    \tikzstyle{measurement} = [very thick, draw = TUMBlue, angle eccentricity = 1.25, angle radius = 3.125em]%
    %
    % Ground
    \node (ground) [ground, yshift = -\groundheight/2, transform shape] {};%
    \draw (ground.north west) -- (ground.north east);%
    %
    \coordinate (origin) at (0, 0);%
    \coordinate [above = 6em of origin, rotate around = {-45/2:(origin)}, transform shape] (A);%
    \coordinate [above = 6em of origin, rotate around = {45/2:(origin)}, transform shape] (B);%
    \coordinate [right = 6em of A, rotate around = {-45/2:(A)}, transform shape] (D);%
    \coordinate [right = -6em of B, rotate around = {45/2:(B)}, transform shape] (E);%
    ...
    \begin{LocalGlsTextBlue}%
        ...
        \draw pic [measurement, "$\gls{lambda}$"] {angle = J--B--E};%
    \end{LocalGlsTextBlue}%
    \pic [draw, thick, transform shape] {right angle = origin--A--G};%
    %
    % Background
    ...
    %
    % Velocity vector
    \draw [arrow, very thick] (A.north) -- (D);%
    \draw [arrow, very thick] (E) -- (B) node[label={[label distance=0]0:$\gls{v}_{\glsxtrshort{com}}$}, very near start, below] {};%
    %
    % Legend
